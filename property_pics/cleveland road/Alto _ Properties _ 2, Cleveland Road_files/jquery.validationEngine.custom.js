 ...
 *
 * WARNING: this library has been customised: the container classname had changed between 2.5 and 2.6.4,
 * so the old class name from 2.5 has been restored to avoid breaking
 * changes in the product's HTML
 *
 * KP: 07/11/2016 ALTO MODIFICATION: pass in noAnimation as a parameter on methods.updatePromptsPosition
 * KD: 29/04/2018 ALTO MODIFICATION: add overload in showPrompt for prompt to be manually closed rather than clear on validation pass
 *                                   and automatically assign z-index of created warnings to current z-index + 1 to prevent messages displaying over modals
 *                                   and fixed issue where position for diaply was calculated incorrectly when parent element was scrolled
 *
 */
(function ($) {
    'use strict';
    ...
    var methods = {
        ...
        validate: function () {
            ...
            if (element.is('form') || element.hasClass('opt-validate')) {
            ...
            } else if (element.is('form') || element.hasClass('opt-validate')) {
                element.removeClass('validating');
            } else {
                // field validation
                var form = element.closest('form, .opt-validate'),
         ...
         */
        updatePromptsPosition: function (event, noAnimation) {
            if (event && this == window) {
                var form = event.data.formElem;
                noAnimation = event.data.noAnimation;
            } else var form = $(this.closest('form, .opt-validate'));
        ...
        showPrompt: function (
            ...
            var form = this.closest('form, .opt-validate');
            var options = form.data('jqv');
            // No option, take default one
            if (!options) options = methods._saveOptions(this, options);
            if (promptPosition) options.promptPosition = promptPosition;
            options.showArrow = showArrow == true;
            options.manuallyClosed = manuallyClosed == true;
        ...
        hide: function () {
            ...
            if ($(this).is('form') || $(this).hasClass('opt-validate')) {
                closingtag =
                    'parentForm' + methods._getClassName($(this).attr('id'));
            } else {
                closingtag =
                    methods._getClassName($(this).attr('id')) + 'formError';
            }
        ...
        _onFieldEvent: function (event) {
            var field = $(this);
            var form = field.closest('form, .opt-validate');
            var options = form.data('jqv');
        ...
        _validateFields: function (form) {
            ...
            var first_err = null;
            form.find('[' + options.validateAttribute + '*=validate]:visible')
                .not(':disabled')
                .each(function () {
                    var field = $(this);
                    var names = [];
            ...
            if (errorFound) {
                if (options.scroll) {
                    ...
                    if (options.isOverflown) {
                        ...
                        var scrollContainer = $(
                            options.overflownDIV + ':not(:animated)'
                        );
        ...
        _past: function (form, field, rules, i, options) {
            var p = rules[i + 1];
            var fieldAlt = $(
                form.find("input[name='" + p.replace(/^#+/, '') + "']")
            );
            var pdate;
        ...
        _buildPrompt: function (field, promptText, type, ajaxed, options) {
            ...
            prompt.addClass(
                'parentForm' +
                    methods._getClassName(
                        field.closest('form, .opt-validate').attr('id')
                    )
            );
            prompt.addClass('formError');

            try {
                prompt.css('z-index', methods._getZIndex(field) + 1);
            } catch (e) {
                console.log('Unable to set z-index; ' + e);
            }
            ...
            if (options.manuallyClosed) prompt.data('manuallyClosed', true);
        ...
        _closePrompt: function (field) {
            var prompt = methods._getPrompt(field);
            if (prompt && !prompt.data('manuallyClosed'))
        ...
        _getPrompt: function (field) {
            var formId = $(field).closest('form, .opt-validate').attr('id');
            var className =
                methods._getClassName(field.attr('id')) + 'formError';
            var match = $(
                '.' +
        ...
        _calculatePosition: function (field, promptElmt, options) {
            ...
            fieldTop = fieldTop + field.offsetParent().scrollTop();
        ...
        _submitButtonClick: function (event) {
            var button = $(this);
            var form = button.closest('form, .opt-validate');
            form.data('jqv_submitButton', button.attr('id'));
        },

        _getZIndex: function (elem) {
