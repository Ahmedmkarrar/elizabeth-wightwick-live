...
jQuery.fn.selectToUISlider = function (settings) {
    ...
    var sliderOptions = {
        ...
        slide: function (e, ui) {
            ...
            var currSelect = jQuery(
                '#' + thisHandle.attr('id').split('handle_')[1]
            );
    ...
    selects.bind('change keyup click', function () {
        ...
        thisHandle
            .parents('.ui-slider:eq(0)')
            .slider('values', handleIndex, thisIndex);
    });
    ...
    selects.each(function (i) {
        ...
        var labelText =
            thisLabel.size() > 0
                ? 'Slider control for ' + thisLabel.text() + ''
                : '';
        var thisLabelId =
            thisLabel.attr('id') ||
        ...
        jQuery(groups).each(function (h) {
            scale.append(
                '<dt style="width: ' +
                    ...
                    this.label +
                    '</span></dt>'
            ); //class name becomes camelCased label
            var groupOpts = this.options;
            jQuery(this.options).each(function (i) {
                var style =
                    inc == selectOptions.length - 1 || inc == 0
                        ? 'style="display: none;"'
                        : '';
                var labelText =
                    options.labelSrc == 'text'
                        ? groupOpts[i].text
                        : groupOpts[i].value;
                scale.append(
                    '<dd style="left:' +
    ...
    sliderComponent.find('.ui-tooltip-pointer-down-inner').each(function () {
        var bWidth = jQuery('.ui-tooltip-pointer-down-inner').css(
            'borderTopWidth'
        );
        var bColor = jQuery(this)
