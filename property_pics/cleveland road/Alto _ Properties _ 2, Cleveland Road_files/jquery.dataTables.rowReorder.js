    ...
    $.fn.rowReordering = function (options) {
        ...
        function fnGetStartPosition(oTable, sSelector) {
            ...
            $(sSelector, oTable).each(function () {
                iPosition = parseInt(
                    oTable.fnGetData(this, properties.iIndexColumn)
                );
        ...
        function fnGetState(oTable, sSelector, id) {
            ...
            if (trPrevious.length > 0) {
                ...
                if (trNext.length > 0) {
                    iNewPosition = parseInt(
                        oTable.fnGetData(trNext[0], properties.iIndexColumn)
                    );
                    if (iNewPosition > iCurrentPosition)
                        //moved back
                        iNewPosition = iNewPosition - 1;
        ...
        function fnMoveRows(
            ...
            $(oTable.fnGetNodes()).each(function () {
                if (sGroup != '' && $(this).attr('data-group') != sGroup)
                    return;
                var tr = this;
                var iRowPosition = parseInt(
                    oTable.fnGetData(tr, properties.iIndexColumn)
                );
                if (iStart <= iRowPosition && iRowPosition <= iEnd) {
                    if (tr.id == id) {
                        oTable.fnUpdate(
                            ...
                            false
                        ); // false = defer redraw until all row updates are done
                    } else {
                        if (sDirection == 'back') {
                            oTable.fnUpdate(
                                iRowPosition + 1,
        ...
        return this.each(function () {
            ...
            $('tbody', oTable).sortable({
                items: 'tr.sortable', //explicitly allow rows to be sortable so the datatable expandable row is not able yo be moved away from its spawning row
                cursor: 'move',
                ...
                update: function (event, ui) {
                    ...
                    if (properties.bGroupingUsed) {
                        sGroup = $(ui.item).attr(
                            properties.sDataGroupAttribute
                        );
                    ...
                    if (properties.sURL != null) {
                        ...
                        properties.fnUpdateAjaxRequest(
                            oAjaxRequest,
                            properties,
                            $dataTable
                        );
