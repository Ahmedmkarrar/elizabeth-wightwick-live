...
(function ($, undf) {
    ...
    $.MixItUp.prototype = {
            ...
            if (
                ...
                if (type === 'sort') {
                    ...
                    if (
                        ...
                        $(self.selectors.sort).removeClass(
                            self.controls.activeClass
                        );
                ...
                if (type === 'filter') {
                    var filter = $button.attr('data-filter'),
                        ndx,
                        seperator =
                            self.controls.toggleLogic === 'or' ? ',' : '';

                    if (!self.controls.toggleFilterButtons) {
                        if (!$button.hasClass(self.controls.activeClass)) {
                            $(self.selectors.filter).removeClass(
                                self.controls.activeClass
                            );
                            trackClick($button, type);
                            self.filter(filter);
        ...
        _updateControls: function (command, multi) {
            ...
            for (var i = 0; i < 2; i++) {
                $el = self.controls.live
                    ...
                    : self['_$' + type + 'Buttons'];
                $el &&
                    update($el, '[data-' + type + '="' + output[type] + '"]');
                type = 'sort';
            }
        ...
        _sort: function () {
            ...
            switch (self._newSort[0].sortBy) {
                ...
                case 'custom':
                    ...
                    break;
                default:
                    self._newOrder = self._startOrder
                        .concat()
                        .sort(function (a, b) {
        ...
        _printSort: function (reset) {
            var self = this;
            var order = reset ? self._startOrder : self._newOrder;
            var targets = self._$parent[0].querySelectorAll(
                self.selectors.target
            );

            if (targets.length == 0) return;

            var nextSibling = targets[targets.length - 1].nextElementSibling;
        ...
        _parseEffects: function () {
            var self = this,
                ...
                parse = function (effect, extract, reverse) {
                    if (self.animation.effects.indexOf(effect) > -1) {
                        if (extract) {
                            var propIndex = self.animation.effects.indexOf(
                                effect + '('
                            );
                            if (propIndex > -1) {
                                var str =
                                        self.animation.effects.substring(
                                            propIndex
                                        ),
                                    match = /\(([^)]+)\)/.exec(str),
                                    val = match[1];
            ...
            effects.opacity = parse('fade')
                ? parse('fade', true).val || '0'
                : '1';
        ...
        _buildState: function (future) {
            ...
            state = {
                activeFilter:
                    self._activeFilter === '' ? 'none' : self._activeFilter,
                activeSort:
        ...
        _goMix: function (animate) {
            ...
            self._$container.trigger('mixStart', [
                self._state,
                futureState,
                self
            ]);
        ...
        _getTargetData: function (el, stage) {
            ...
            if (self.animation.animateResizeTargets) {
                ...
                el.dataset[stage + 'MarginBottom'] = parseInt(
                    elStyle.marginBottom
                );
                el.dataset[stage + 'MarginRight'] = parseInt(
                    elStyle.marginRight
                );
        ...
        _getOrigMixData: function () {
            var self = this,
                parentStyle = !self._suckMode
                    ...
                    : { boxSizing: '' },
                parentBS =
                    parentStyle.boxSizing ||
                    parentStyle[self._vendor + 'BoxSizing'];
        ...
        _prepTargets: function () {
            var self = this,
                transformCSS = {
                    _in: self._getPrefixedCSS(
                        'transform',
                        self.effects.transformIn
                    ),
                    _out: self._getPrefixedCSS(
                        'transform',
                        self.effects.transformOut
                    )
                };
            ...
            var self = this,
                n =
                    typeof self.animation.staggerSequence === 'function'
                        ? self.animation.staggerSequence.call(
                              self._domNode,
                              i,
                              self._state
                          )
                        : i,
                delay = self.animation.stagger
                    ? n * self.animation.staggerDuration
                    : 0;
        ...
        multiMix: function () {
            ...
            if (!self._mixing) {
                if (self.controls.enable && !self._clicking) {
                    self.controls.toggleFilterButtons &&
                        self._buildToggleArray();
                    self._updateControls(
                        args.command,
                        self.controls.toggleFilterButtons
        ...
        insert: function () {
            var self = this,
                ...
                callback =
                    typeof args.callback === 'function' ? args.callback : null,
                frag = document.createDocumentFragment(),
                target = (function () {
                    self._refresh();
                    ...
                    if (self._$targets.length) {
                        return args.index < self._$targets.length ||
                            ...
                            : self._$targets[self._$targets.length - 1]
                                  .nextElementSibling;
                    } else {
