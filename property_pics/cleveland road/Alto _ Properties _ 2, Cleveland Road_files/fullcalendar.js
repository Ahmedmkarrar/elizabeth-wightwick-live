/*## Changes */
// option > 'multi' added.
// option > 'officeStartHour' & 'officeEndHour' added
// option > 'customDays' added
...
(function ($, undefined) {
    ...
    function Calendar(element, options, eventSources) {
        ...
        function gotoDate(year, month, dateOfMonth, customDays, noRender) {
            if (customDays) {
                options.customDays = customDays;
            }
    ...
    function Header(calendar, options) {
        ...
        function render() {
            ...
            if (sections) {
                element = $(
                    "<table class='fc-header' style='width:100%'></table>"
                ).append(
                    $('<tr></tr>')
    ...
    function MonthView(element, calendar) {
        ...
        function render(date, delta) {
            ...
            var rowCnt = Math.round(
                (visEnd - visStart) / (DAY_MS * opt('customDays'))
            );
            if (opt('weekMode') == 'fixed') {
                addDays(
                    visEnd,
                    (opt('customDays') - 1 - rowCnt) * opt('customDays')
                );
                rowCnt = opt('customDays') - 1;
            }
    ...
    function BasicWeekView(element, calendar) {
        ...
        function render(date, delta) {
            var customDays = opt('customDays');
            if (delta) {
                addDays(date, delta * customDays);
            }
            var start;
            if (opt('customPeriod')) {
                //Don't do anything fancy with the new date.  Just use it.
                start = cloneDate(date, true);
            } else {
                start = addDays(
                    cloneDate(date),
                    -(
                        (date.getDay() - opt('firstDay') + customDays) %
                        customDays
                    )
                );
            }
            var end = addDays(cloneDate(start), customDays);
            var visStart = cloneDate(start);
            var visEnd = cloneDate(end);
            ...
            t.visStart = visStart;
            t.visEnd = visEnd;
            renderBasic(1, 1, weekends ? customDays : 5, false);
        }
    }
    ...
    function BasicView(element, calendar, viewName) {
        ...
        function buildSkeleton(maxRowCnt, showNumbers) {
            ...
            s =
                ...
                '<tr>';
            for (i = 0; i < colCnt; i++) {
                s += "<th class='fc- " + headerClass + "'></th>"; // need fc- for setDayID
            }
            s += '</tr>' + '</thead>' + '<tbody>';
        ...
        function renderDayOverlay(
            ...
            for (var i = 0; i < rowCnt; i++) {
                ...
                addDays(rowStart, opt('customDays'));
                addDays(rowEnd, opt('customDays'));
            }
        }
        ...
        function dateCell(date) {
            return {
                row: Math.floor(dayDiff(date, t.visStart) / opt('customDays')),
        ...
        function _cellDate(row, col) {
            return addDays(
                ...
                row * opt('customDays') + col * dis + dit
            );
    ...
    function BasicEventRenderer() {
        ...
        function compileSegs(events) {
            ...
            for (i = 0; i < rowCnt; i++) {
                ...
                addDays(d1, opt('customDays'));
                addDays(d2, opt('customDays'));
            }
            return segs;
        }
        ...
        function draggableDayEvent(event, eventElement) {
            ...
            var dayDelta;
            var $view = $(eventElement).closest('.fc-view');
    ...
    function AgendaWeekView(element, calendar) {
        ...
        function render(date, delta) {
            ...
            t.visStart = visStart;
            t.visEnd = visEnd;
            renderAgenda(weekends ? customDays : 5);
        }
    }
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function buildSkeleton() {
            ...
            s =
                ...
                "'>&nbsp;</th>";
            for (i = 0; i < colCnt; i++) {
                s += "<th class='fc- fc-col" + i + ' ' + headerClass + "'></th>"; // fc- needed for setDayID
            }
            s +=
            ...
            slotScroller = //###added a class to this div - "slot-scroller"
                $(
                    '<div class="slot-scroller" style=\'position:absolute;width:100%;overflow-x:hidden;overflow-y:' +
                        (opt('multi') || opt('printable') ? 'hidden' : 'auto') +
                        ";'></div>"
                ).appendTo(slotLayer);
            ...
            slotCnt = 0;
            var osh = opt('officeStartHour');
            var oeh = opt('officeEndHour');
            var osm = opt('officeStartMinute');
            var oem = opt('officeEndMinute');
            for (i = 0; d < maxd; i++) {
                minutes = d.getMinutes();
                var hour = d.getHours(); //sjd added

                var currentMinutesFromMidnight = hour * 60 + minutes;
                var startMinutesFromMidnight = osh * 60 + osm;
                var endMinutesFromMidnight = oeh * 60 + oem;

                var isOfficeHour =
                    currentMinutesFromMidnight >= startMinutesFromMidnight &&
                    currentMinutesFromMidnight < endMinutesFromMidnight;

                var dataAttributeNameDiarySlot =
                    ...
                    "'";
                s +=
                    "<tr class='{0} fc-slot fc-slot".format(
                        isOfficeHour ? '' : 'ooh'
                    ) +
                    i +
                    ' ' +
        ...
        function setHeight(height, dateChanged) {
            ...
            if (opt('printable')) {
                slotScroller.height(6666);
                slotScroller.css('position', 'relative');
            } else {
        ...
        function setWidth(width) {
            ...
            var slotTableWidth = slotScroller.width();
        ...
        function resetScroll() {
            //If the calendar is in multi mode, the slot scroller will have been moved.  Re-point it.
            if (opt('multi')) {
                //SJD added
                if (!slotScroller.parent().hasClass('agenda')) {
                    slotScroller = element
                        .closest('.calendar')
                        .find('.slot-scroller');
                }
            }
            ...
            function scroll() {
                if (opt('multi') == false) {
                    ...
                    //Multi-mode Scroll the agenda (slot container) instead.
                    slotScroller.parent().scrollTop(top);
                }
            }
            scroll();
    ...
    function AgendaEventRenderer() {
        ...
        function slotSegHtml(event, seg) {
            ...
            var dataAttributeNameDiaryAppointment =
            ...
            html +=
                ...
                skinCss +
                "' " +
                dataAttributeNameDiaryAppointment +
                '>' +
        ...
        function draggableSlotEvent(event, eventElement, timeElement) {
            ...
            var $scroller = $(eventElement).closest('.slot-scroller');
            var scrollStartOffset;
            ...
            eventElement.draggable({
                ...
                revertDuration: opt('dragRevertDuration'),
                start: function (ev, ui) {
                    // Get scroll position of scroller
                    scrollStartOffset = $scroller.scrollTop();
                    //
                    trigger('eventDragStart', eventElement, event, ev, ui);
                    hideEvents(event, eventElement);
                ...
                drag: function (ev, ui) {
                    // Reposition element after scroll events
                    updateDragOffset(ui);
                    //
                    minuteDelta =
                        Math.round(
            ...
            function updateDragOffset(ui) {
                var offset = scrollStartOffset - $scroller.scrollTop();
    ...
    function OverlayManager() {
        ...
        function renderOverlay(rect, parent) {
            ...
            if (!e) {
                e = $(
                    "<div class='fc-cell-overlay' style='position:absolute;z-index:3'></div>"
                );
            }
