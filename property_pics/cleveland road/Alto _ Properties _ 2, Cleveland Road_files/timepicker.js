((function(factory) {
    ...
    var methods = {
        ...
        getTime: function () {
            ...
            return new Date(
                today.valueOf() + _time2int(_getTimeValue(self)) * 1000
            );
        },
        ...
        setTime: function (value) {
            ...
            var prettyTime = _int2time(
                _time2int(value),
                self.data('timepicker-settings').timeFormat
            );
    ...
    function _closeHandler(e) {
        ...
        if (
            input.length === 0 &&
            target.closest('.ui-timepicker-list').length === 0
        ) {
            methods.hide();
        ...
        switch (e.keyCode) {
            ...
            case 38: // up
                ...
                if (!selected.length) {
                    ...
                    if (
                        selected.prev().position().top < selected.outerHeight()
                    ) {
                        list.scrollTop(
                            list.scrollTop() - selected.outerHeight()
                        );
                    }
                }
            ...
            case 40: // down
                ...
                if (selected.length === 0) {
                    ...
                    if (
                        selected.next().position().top +
                            2 * selected.outerHeight() >
                        list.outerHeight()
                    ) {
                        list.scrollTop(
                            list.scrollTop() + selected.outerHeight()
    ...
    $.fn.timepicker = function (method) {
        if (methods[method]) {
            ...
            $.error(
                'Method ' + method + ' does not exist on jQuery.timepicker'
            );
        }
    };
});
