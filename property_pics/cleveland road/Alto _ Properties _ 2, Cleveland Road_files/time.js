...
TimeSpan = function (days, hours, minutes, seconds, milliseconds) {
    ...
    if (arguments.length == 5) {
        ...
        this.days =
            Math.floor(this.milliseconds / (24 * 60 * 60 * 1000)) * orient;
...
TimeSpan.prototype.add = function (timeSpan) {
    return timeSpan === null
        ? this
        : this.addSeconds(timeSpan.getTotalMilliseconds() / 1000);
};
...
TimeSpan.prototype.toString = function (format) {
    function _toString() {
        if (this.days() !== null && this.days() > 0) {
            ...
            return (
                this.hours() + ':' + p(this.minutes()) + ':' + p(this.seconds())
            );
        }
    }
    function p(s) {
    ...
    var self = this;
    return format
        ? format.replace(/d|dd|HH|H|hh|h|mm|m|ss|s|tt|t/g, function (format) {
              switch (format) {
                  case 'd':
                      return self.days();
                  case 'dd':
...
var TimePeriod = function (
    ...
    if (
        ...
        if (orient == +1) {
            while (
                date1 < date2 &&
                date1.clone().addDays(date1.getDaysInMonth()) < date2
            ) {
                date1.addMonths(1);
                this.months++;
            }
