...
//##changes
// sjd 10/02/2013: Added modelTypeProp
// sjd 10/02/2013: Default keyDelay changed from 400 to 250
// sjd 02/08/2013: Added udf1, 2 & 3
// ngm 07/09/2015: Added a scrollIntoView() for the results else they drop off the bottom when DOWN key pressed
        ...
        if (d_fetcher) {
            return this.each(function (x) {
                ...
                function add_selected_item(data, num) {
                    ...
                    var composedItemHtml = '<li class="as-selection-item" id="as-selection-' + num +
                        '" data-modelType="' + data[opts.modelTypeProp] +
                        '" data-udf1="' + data[opts.udf1] +
                        '" data-udf2="' + data[opts.udf2] +
                        '" data-udf3="' + data[opts.udf3] +
                        '" data-consents="' + JSON.stringify(data[opts.consents]).replace(/\"/g, '&quot;') +
                        '" data-value="' + data[opts.selectedValuesProp] + '"></li>';


                    var item = $(composedItemHtml).click(function () {
                            opts.selectionClick.call(this, $(this));
                            selections_holder.children().removeClass("selected");
                            $(this).addClass("selected");
                        }).mousedown(function () { input_focus = false; });

                    if (data[opts.branchIdProp]) {
                        item.attr('data-branch-id', data[opts.branchIdProp]);
                    }

                    item.html(data[opts.selectedItemProp]);

                    if (opts.allowDelete) {
                        var close = $('<a class="as-close"></a>').click(function () {  //SJD - emptied, changed for image.
                ...
                function moveSelection(direction) {
                    if ($(":visible", results_holder).length > 0) {
                        ...
                        start.scrollintoview({ direction: 'y' });
                    }
                }
