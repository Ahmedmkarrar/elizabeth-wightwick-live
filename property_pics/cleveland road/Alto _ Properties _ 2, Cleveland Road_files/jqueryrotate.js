...
(function ($) {
    var supportedCSS,
        ...
        toCheck = 'transformProperty WebkitTransform OTransform'.split(' '); //MozTransform <- firefox works slower with css!!!
    ...
    jQuery.fn.extend({
        ...
        rotateAnimation: function (parameters) {
            if (this.length === 0 || typeof parameters == 'undefined') return;
            if (typeof parameters == 'number')
                parameters = { animateAngle: parameters };
            var returned = [];
            for (var i = 0, i0 = this.length; i < i0; i++) {
                var element = this.get(i);
                if (typeof element.Wilq32 == 'undefined')
    ...
    Wilq32.PhotoEffect = (function () {
        function setupParameters(img, parameters) {
            ...
            this._parameters.animateAngle =
                typeof parameters.animateAngle == 'number'
                    ? parameters.animateAngle
                    : this._angle;
        }
        if (supportedCSS) {
            return function (img, parameters) {
                ...
                if (this._parameters.angle != this._parameters.animateAngle)
                    this.rotateAnimation(this._parameters);
            };
        } else {
            return function (img, parameters) {
                setupParameters.call(this, img, parameters);
    ...
    Wilq32.PhotoEffect.prototype = {
            ...
            if (IE)
                return function () {
                    ...
                    this._container.setAttribute(
                        'coordsize',
                        width - 1 + ',' + (height - 1)
                    ); // This -1, -1 trying to fix that ugly problem
        ...
        _animate: function () {
            var checkEnd =
                !!Math.round(
                    this._angle * 100 - this._parameters.animateAngle * 100
                ) == 0 && !!this._timer;
